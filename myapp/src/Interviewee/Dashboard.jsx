    ...
    <div>
      ...
      <div className="d-flex">
        <ul id="sidebar">
          ...
          >
            <a href={`/${interview}/${employee}`}>All</a>
          </li>
          {range(qnum).map((num) => (
            <li
              key={num + 1}
            ...
            >
              <a href={`/${interview}/${employee}/${num + 1}`}>{num + 1}</a>
            </li>
          ))}
        </ul>
        <main>
            ...
            <Route exact path={path}>
              {range(qnum).map((num) => (
                <QuestionBox
                  key={num}
                  qno={num + 1}
                  qname={questions[num + 1]["title"]}
                  testsPassed={questions[num + 1]["testcases"]["total"]}
                  qlink={`/${interview}/${employee}/${num + 1}`}
                />
              ))}
            </Route>
            
            <Route exact path={`${path}/:number`}> {/* number refers to question number */}
              <QuestionPage
                onUpdateIndex={(number) => setActiveIndex(number)}
                questions={questions}
                interview={interview}
                employee={employee}
              ></QuestionPage>
